...
impl Cond {
    ...
    pub fn integer_great_than_or_equal_to(field_name: &str, value: i64) -> Cond {
        Cond::IntegerGreaterThanOrEqualTo {
            field_name: field_name.to_string(),
            value,
        }
    }
    ...
    pub fn is_row_id_equal_to<T: Schema>(record: &T) -> Option<Cond> {
        ...
        match row_id_field_and_value {
            Some((field, row_id)) => match row_id {
                crate::RowID::NEW => None,
                crate::RowID::ID(value) => Some(Cond::IntegerEqualTo {
                    field_name: field,
                    value: value,
                }),
            },
